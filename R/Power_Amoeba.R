...
est_img_1 <- JPLLK_surface(x,3,plot = F)$fitted                                # Jump preserving estimate of image 1
est_img_2 <- JPLLK_surface(x_1,3,plot = F)$fitted                                # Jump preserving estimate of image 2
edge_1 <- stepEdge(est_img_1,h,(qnorm(0.999,0,1)*JPLLK_surface(est_img_1,3)$sigma),degree=0,plot = F)
edge_2 <- stepEdge(est_img_2,h,(qnorm(0.999,0,1)*JPLLK_surface(est_img_2,3)$sigma),degree=0,plot = F)
...
for(q in 1:epoch){
  set.seed(2025+q)
  nimg_1.star <-  est_img_1 + matrix(rnorm(nrow(x)*ncol(x),0,sigma1_hat),nrow=nrow(x),ncol=ncol(x))
  nimg_2.star <-  translation(rotateImage(est_img_1,runif(1,0,20) , threads = 1)
                              ,shift_rows = sample(1:10,1),shift_cols = sample(1:10,1))
  + matrix(rnorm(nrow(x)*ncol(x),0,sigma2_hat),nrow=nrow(x),ncol=ncol(x))
...
for(q in 1:epoch){
  ...
  set.seed(2025+q)
  nimg_1.star <-  est_img_2 + matrix(rnorm(nrow(x)*ncol(x),0,sigma1_hat),nrow=nrow(x),ncol=ncol(x))
  nimg_2.star <-  translation(rotateImage(est_img_1,runif(1,0,20) , threads = 1)
                              ,shift_rows = sample(1:10,1),shift_cols = sample(1:10,1))
  + matrix(rnorm(nrow(x)*ncol(x),0,sigma2_hat),nrow=nrow(x),ncol=ncol(x))
