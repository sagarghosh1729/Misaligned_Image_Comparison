...
local_change_detection<- function(image_1_path, image_2_path){
  ...
  ggplot() +
    ...
    geom_segment(data = points_df, aes(x = x - cross_size, xend = x + cross_size,
                                       y = y, yend = y), color = "red", size = 1.0) + # Decreased size slightly for cleaner look
    geom_segment(data = points_df, aes(x = x, xend = x,
                                       y = y - cross_size, yend = y + cross_size), color = "red", size = 1.0) + # Decreased size slightly for cleaner look
