...
# 1. Euclidean Invariant Metric
  ...
  choose_landmarks <- function(P,
    ...
    pick_for_angle <- function(theta) {
      ...
      # Expand tolerance until we find candidates, preferring the farthest 
      #along the ray
      tol_k <- tol
      chosen <- NA_integer_
      ...
      # If still nothing, fall back to nearest-angle point (dense edges usually 
      #make this rare)
      if (is.na(chosen)) {
        phi <- atan2(V[, 2], V[, 1])
...
x_1[,1:56]=0
nimg1 <- x_1 + matrix(rnorm(nrow(x_1)*ncol(x_1),0,0.01),nrow=nrow(x_1),
                      ncol=ncol(x_1))
